...
testRecords (void)
{
    ...
    OP_TRUE(stringToValue("sjjjj"), value, valueEquals, "second attr");
    freeVal(value);
...
testCreateTableAndInsert (void)
{
    ...
    int numInserts = 10, i; //changed to 10
    Record *r;
    RID *rids;
    Schema *schema;
    ...
    for(i = 0; i < 2000; i++) // changed condition value
    {
...
testMultipleScans(void)
{
    ...
    int numInserts = 11, i, scanOne=0, scanTwo=0; //changed to 11
    Record *r;
    RID *rids;
    Schema *schema;
...
testInsertManyRecords(void)
{
    ...
    TestRecord realInserts[5000]; //changed array size
    TestRecord updates[] = {
        {4444, "iiii", 6} //changed
    };
    int numInserts = 5000, i; //changed value
    int randomRec = 4444; //chnaged
    Record *r;
    RID *rids;
    Schema *schema;
