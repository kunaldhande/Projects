...
def bfs(graph, root, max_depth):
    ...
    queue = deque()
    path = []
    node2distances, node2num_paths, node2parents, visited = defaultdict(lambda: -1), defaultdict(int), defaultdict(list), defaultdict(bool)

    node2distances[root], node2num_paths[root] = 0, 1
    queue.append(root)
    ...
    while queue:
        ...
        if vertex not in path:
            ...
            for node in graph.neighbors(vertex):
                if node2distances[node] == -1:
                    node2distances[node] = node2distances[vertex] + 1
                    visited[node] = True
                    queue.append(node)

                if visited[node] and node2distances[node] > node2distances[vertex]:
                    node2num_paths[node] += 1
                    node2parents[node].append(vertex)
...
def bottom_up(root, node2distances, node2num_paths, node2parents):
    ...
    """
    ###TODO
    result, score = defaultdict(list), defaultdict()
    for node, parents in node2parents.items():
        for innernode in parents:
            result[innernode].append(node)

    queue, q = deque(), deque()
    queue.append(root)

    # Calculating score for leaf nodes
    while queue:
        vertex = queue.popleft()
        if vertex in result:
            queue.extend(result[vertex])
            q.append(vertex)
        else:
            score[vertex] = 1.0
    ...
    while q:
        vertex = q.pop()
        for node in result[vertex]:
            if node in score:
                if vertex not in score: score[vertex] = 1.0
                score[vertex] += score[node]/node2num_paths[node]

    # Calculating credit score for edges
    result = defaultdict()
    for node, parents in node2parents.items():
        for innernode in parents:
            result[tuple(sorted([innernode, node]))]= score[node]/node2num_paths[node]
    return result
...
def approximate_betweenness(graph, max_depth):
    ...
    for node in graph.nodes():
        node2distances, node2num_paths, node2parent = bfs(graph, node, max_depth)
        credit = merge_with(credit, bottom_up(node, node2distances, node2num_paths, node2parent))

    return {k: credit[k]/2 for k in credit.copy()}
...
def partition_girvan_newman(graph, max_depth):
    ...
    ###TODO
    G = graph.copy()
    if G.order() == 1: return [G]

    betweenness = sorted(approximate_betweenness(G, max_depth).items(), key=lambda x:(-x[1], x[0][0],x[0][1]), reverse = True)
    components = [c for c in nx.connected_component_subgraphs(G)]

    # Remove edge with max betweenness from graph two get 2 dictinct graphs
...
def get_subgraph(graph, min_degree):
    ...
    ###TODO
    g = graph.copy()
    nodes_to_draw = [name for name, degree in g.degree().items() if degree >= min_degree]
...
def score_max_depths(graph, max_depths):
    ...
    for max_depth in max_depths:
        cluster =  partition_girvan_newman(graph, max_depth)
        cluster = sorted(cluster, key=lambda x: sorted(x.nodes())[0])
        cut_v.append((max_depth, norm_cut(cluster[0].nodes(), cluster[1].nodes(), graph)))
    return cut_v
...
def make_training_graph(graph, test_node, n):
    ...
    ###TODO
    g = graph.copy()
    edges_to_rem = sorted(g.edges(test_node))[:n]
...
def path_score(graph, root, k, beta):
    ...
    ###TODO
    scores = []
    node2distances, node2num_paths, _ = bfs(graph, root, math.inf)

    for node in node2distances:
        if node != root and not graph.has_edge(root, node):
            score = pow(beta , node2distances[node]) * node2num_paths[node]
...
def evaluate(predicted_edges, graph):
    ...
    """
    ###TODO
    return len([x for x in predicted_edges if graph.has_edge(*x)]) / len(predicted_edges)
