...
def plot_sorted_accuracies(results):
    ...
    """
    ###TODO
    y_axis = [result['accuracy'] for result in reversed(results)]
...
def mean_accuracy_per_setting(results):
    ...
    for result in results:
        res['punct=' + str(result['punct'])].append(result['accuracy'])
        res['min_freq=' + str(result['min_freq'])].append(result['accuracy'])
        res['features=' + " ".join([func.__name__ for func in result['features']])].append(result['accuracy'])

    return sorted([(np.mean(res[result]), result) for result in res], key= lambda x:-x[0])
...
def top_coefs(clf, label, n, vocab):
    ...
    """
    ###TODO
    terms = np.array([x[0] for x in sorted(vocab.items(), key=lambda x: x[1])])
    coef = clf.coef_[0]
    srted = np.argsort(coef)

    if label:
        pos = srted[::-1][:n]
        return [(t, v) for t, v in zip(terms[pos], coef[pos])]
    else:
        neg = srted[:n]
        return [(t, abs(v)) for t, v in zip(terms[neg], coef[neg])]
...
def parse_test_data(best_result, vocab):
    ...
    tokens = [tokenize(doc, best_result['punct']) for doc in docs]
    X, vocab = vectorize(tokens, best_result['features'], best_result['min_freq'], vocab)
...
def print_top_misclassified(test_docs, test_labels, X_test, clf, n):
    ...
    for i in range(len(predicted)):
        if predicted[i] != test_labels[i]:
            dict_value = dict(doc= test_docs[i], probas= predicted_proba[i], probability=predicted_proba[i][predicted[i]], truth= test_labels[i], predicted= predicted[i])
            list_dict.append(dict_value)

    for d in sorted(list_dict, key=lambda k: k['probability'], reverse=True)[:n]:
        print("\ntruth=", d['truth'], "predicted=", d['predicted'] , "proba=", d['probability'])
        print(d['doc'])
